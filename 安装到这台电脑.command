...
cp "$script_dir/点我立即备份Codex.command" "$dest/点我立即备份Codex.command"
cp "$script_dir/卸载自动备份.command" "$dest/卸载自动备份.command"
cp "$script_dir/怎么用.md" "$dest/怎么用.md"

chmod 755 "$dest/codex_backup.sh" "$dest/backup-now.command" "$dest/uninstall.command" "$dest/点我立即备份Codex.command" "$dest/卸载自动备份.command"
...
<dict>
  ...
  <string>$dest/自动备份.log</string>
  <key>StandardErrorPath</key>
  <string>$dest/自动备份-error.log</string>
